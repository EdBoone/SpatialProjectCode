...
tensorProductEstimate<-function(Y,X,X_hout,NMCMC=1500,BURNIN=200,M=5,K=5){
  ...
  TPs_p <- array(1,dim=c(nrow( X_hout ), 2, M, 2 ))  # For mean of predictions
  ...
  for(ijk in 1:NTOTITER){
      ...
      m_mean1_p = as.matrix( TPscale[1,1]*apply(TPs_p[,,1,1],1,prod))
      ...
      pp1_rho  <- temp[2]
      ###
      Pred1 <- pred_Mean_Matern( X = X[,1], 
                        X_new = X_hout[,1], 
                        m_X = rep(0, length( X[,1 ] ) ), 
                        m_new = rep(0, length( X_hout[,1] ) ), 
                        obsv_X = m_mean1, 
                        var=pp1_sig2, 
                        ls = pp1_rho )
      #Pred1V <- pred_COV_matern(X = X[,1],
      #                  X_new = X_hout[,1],
      #                  var = pp1_sig2,
      #                  ls = pp1_rho )
